...
class Course extends Controller
{
	...
	public function course()
	{
		...
		$course = Db::table('micro_course')
					->alias('course')
					->join('micro_stage stage','course.stage_id = stage.stage_id')
					->join('micro_stage_child stage_child','course.stage_child_id=stage_child.stage_child_id')
					...
					// ->toArray();
					// ->select();
		$stage = Db::table('micro_stage')
					->select(); 
		//课程视图
		return $this->fetch('course',['data'=>$course,'stage'=>$stage]);
	}
	...
	public function courseAdd()
	{
		...
		$stage = Db::table('micro_stage')
					->alias('stage')     
					// ->join('micro_stage_child child','stage.stage_id = child.stage_id')
					->select(); 
		// 处理数组
		// foreach ($data as $k => $v) {
		// 	$stage[$v['stage_name']][] = $v;
		// }
		//  教师信息  
        ...
        return $this->fetch('course_add',['stage' => $stage,'teacher'=>$teacher]);
	}
    ...
    public function uploads($name)
    {
		...
		$image_path = ROOT_PATH . 'public/uploads';
		$image_info = $files->move($image_path);
		// $savename = collection($image_info)->toArray();
		$savename = '/uploads/'.$image_info->getSaveName();
	...
	public function courseAjax()
	{
		...
		$stage_child = Db::table('micro_stage_child')
	...
	{
		$stage_child_id = Request::instance()->get('stage_child_id');
		$subject = Db::table('micro_stage_child_subject')
						->alias('child_subject')
						->join('micro_subject subject','child_subject.subject_id = subject.subject_id')
						->where('stage_child_id',$stage_child_id)
						->select();
		return $subject;
	}
	...
	public function courseUpd()
	{
		...
		$course = Db::table('micro_course')
					...
					->select();
		// var_dump($course);die;
		return  $this->fetch('course_upd',['course'=>$course[0]]);
	}
